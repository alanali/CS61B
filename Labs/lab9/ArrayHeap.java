...
public class ArrayHeap<T> {
    ...
    public T peek() {
        ...
        for (int i = 1; i < size(); i++) {
            small = minPriorityIndex(small, i);
        }
        return contents.get(small).item();
    }
    ...
    private void bubbleDown(int index) {
        int lc = getLeftOf(index);
        int rc = getRightOf(index);
        int small;
        if (lc >= contents.size() && rc >= contents.size()) {
            return;
        } else if (lc >= contents.size()) {
            small = rc;
        } else if (rc >= contents.size()) {
            small = lc;
        } else {
            small = minPriorityIndex(lc, rc);
        }
    ...
    public void changePriority(T item, double priority) {
        ...
        for (int i = 1; i < contents.size(); i++) {
            double pri = contents.get(i).priority();
            if (contents.get(i)._item.equals(item)) {
