...
class Board {
    ...
    void clear() {
        ...
        for (int r = 0; r < _board.length; r++) {
            if (r < 2 * EXTENDED_SIDE ||  r >= 9 * EXTENDED_SIDE) {
                _board[r] = BLOCKED;
            } else if (r % 11 <= 1 || r % 11 >= 9) {
                _board[r] = BLOCKED;
            } else if (r == 8 * EXTENDED_SIDE + 8
                    || r == 2 * EXTENDED_SIDE + 2) {
                _board[r] = BLUE;
                incrPieces(BLUE, 1);
            } else if (r == 2 * EXTENDED_SIDE + 8
                    || r == 8 * EXTENDED_SIDE + 2) {
                _board[r] = RED;
                incrPieces(RED, 1);
            } else {
    ...
    PieceColor getWinner() {
        if (numPieces(RED) == 0) {
            ...
            return EMPTY;
                ...
                && numPieces(RED) == numPieces(BLUE)) {
            return EMPTY;
        } else if (!canMove(RED) && !canMove(BLUE) && numPieces(RED)
                > numPieces(BLUE)) {
            return RED;
        } else if (!canMove(RED) && !canMove(BLUE) && numPieces(RED)
                < numPieces(BLUE)) {
    ...
    boolean legalMove(Move move) {
        if (move == null) {
            ...
            if (_whoseMove != _board[move.fromIndex()]) {
                return false;
    ...
    void makeMove(Move move) {
        ...
        addUndo(move.toIndex());
        _board[move.toIndex()] = _whoseMove;
        for (int i = move.toIndex() - 12; i < move.toIndex() - 9; i++) {
    ...
    void undo() {
        _whoseMove = _whoseMove.opposite();
        Move mistake = _allMoves.remove(_allMoves.size() - 1);
        while (!_undoSquares.empty()) {
            ...
            PieceColor piec = _undoPieces.pop();
            if (_board[ind] == RED) {
                incrPieces(RED, -1);
            } else if (_board[ind] == BLUE) {
                incrPieces(BLUE, -1);
            }
            incrPieces(piec, 1);
            _board[ind] = piec;
        }
        incrPieces(_board[mistake.toIndex()], -1);
        _board[mistake.toIndex()] = EMPTY;
        announce();
    }
    ...
    void setBlock(char c, char r) {
        ...
        char cReflect = c;
        char rReflect = r;
        char[] let = new char[] {'a', 'b', 'c', 'd', 'e', 'f', 'g'};
        char[] num = new char[] {'1', '2', '3', '4', '5', '6', '7'};
        for (int i = 0; i < let.length; i += 1) {
            if (let[i] == c) {
                cReflect = let[let.length - i - 1];
            }
            if (num[i] == r) {
                rReflect = num[num.length - i - 1];
            }
        }
        _board[index(c, r)] = BLOCKED;
        _board[index(cReflect, r)] = BLOCKED;
        _board[index(c, rReflect)] = BLOCKED;
        _board[index(cReflect, rReflect)] = BLOCKED;
