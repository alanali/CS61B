...
class Move {
    ...
    private Move(int col0, int row0, int col1, int row1) {
        ...
        int diff = abs(_fromIndex - _toIndex);
        if (diff == 1 || diff == 10 || diff == 11 || diff == 12) {
            _isExtend = true;
            _isJump = false;
        } else {
            _isExtend = false;
    ...
    public String toString() {
        ...
        if (isPass()) {
            ...
            ret += Character.toString(col0()) + Character.toString(row0()) + "-"
                    + Character.toString(col1()) + Character.toString(row1());
        }
        return ret;
    }
