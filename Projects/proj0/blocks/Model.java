...
class Model {
    ...
    boolean placeable(Piece piece, int row, int col) {
        ...
        for (int r = row; r < row + piece.height(); r++) {
            for (int c = col; c < col + piece.width(); c++) {
                if (piece.get(r - row, c - col) && get(r, c)) {
                    return false;
                }
            }
        }
    ...
    void clearFilledLines() {
        ...
        _score += scoreClearedLines(nrows, ncols) - overlap
                + scoreClearedLines(nrows, ncols) * _streakLength;
    }
