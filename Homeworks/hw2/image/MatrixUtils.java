...
public class MatrixUtils {
        ...
        for (int r = 1; r < m.length; r++) {
            for (int c = 0; c < m[0].length; c++) {
                ...
                for (int deltaC = -1; deltaC <= 1; deltaC += 1) {
                    best =  Math.min(Math.min(helperGetCell(m, r - 1, c), helperGetCell(m, r - 1, c - 1)),
                            helperGetCell(m, r - 1, c + 1));
                }
                m[r][c] += best;
            }
        }
    ...
    private static double helperGetCell(double[][] e, int r, int c) {
        if (r >= 0 && r < e.length && c >= 0 && c < e[r].length) {
            return e[r][c];
        } else {
    ...
    public static double[][] accumulate(double[][] m, Orientation orientation) {
        if (orientation == Orientation.VERTICAL) {
            ...
            return matrixTranspose(accumulateVertical(matrixTranspose(m)));
        }
    }
    ...
    public static int[] findVerticalSeam(double[][] m) {
        ...
        for (int r = 0; r < m.length; r++) {
            sol[r] = smallestIndex(m[r], low, high);
            if (sol[r] <= 1) {
                low = 0;
            } else if (sol[r] >= m[0].length - 1) {
                high = m[0].length - 1;
            }
            else {
                low = sol[r] - 1;
    ...
    public static int smallestIndex(double[] g, int lo, int hi) {
        int index = 0;
        double min = g[lo];
        int sol = 0;
    ...
    public static int[] findSeam(double[][] m, Orientation orientation) {
        if (orientation == Orientation.VERTICAL) {
            ...
            return findVerticalSeam(matrixTranspose(m));
        }
    }
